...
def computeDelta(wt, X, Xi):
    ...
    def similarity(a, b):
        ...
        # return cos_sim
        return sum(a_norm * b_norm) / (len(X) * a_std * b_std)

    num = 0.0
    denom = 0.0
    for index, xi in Xi.iterrows():
        sim = similarity(xi.T[0:-1], X[0:-1])
        num += xi['Yi'] * math.exp(wt * sim)
        denom += math.exp(wt * sim)
...
# YOUR CODE HERE
model = smf.OLS(trainData['deltaP'], add_constant(trainData[['deltaP90', 'deltaP180', 'deltaP360']])).fit()
...
for i in xrange(0,len(test_90.index)):
    testDeltaP90 = np.append(testDeltaP90, computeDelta(weight,test_90.iloc[i],test_90))
for i in xrange(0,len(test_180.index)) :
  testDeltaP180 = np.append(testDeltaP180, computeDelta(weight,test_180.iloc[i],test_180))
for i in xrange(0,len(test_360.index)) :
  testDeltaP360 = np.append(testDeltaP360, computeDelta(weight,test_360.iloc[i],test_360))
...
result = model.predict(add_constant(testData[['deltaP90', 'deltaP180', 'deltaP360']]))
